if Meteor.is-client
  Template.signup_or_register.events = {
    ...
    'click .signup_login': (ev, tpl) !->
        ...
        if user then Session.set 'current-user', user else alert "There is no #{username} or you may have wrong password."
    ...
    'click .register_register': (ev, tpl) ->
        ...
        if user then alert "There already have #{username}, why not change an username?" else
